...
export default function AttemptClient({ attemptId }: { attemptId: string }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {/* --- HEADER --- */}
      <header className="sticky top-0 z-50 border-b border-indigo-100 bg-white/80 backdrop-blur-md shadow-sm">
            ...
            {section.passage ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 items-start">
                ...
                <div className="lg:sticky lg:top-24 space-y-4">
                  <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm overflow-hidden ring-4 ring-slate-50/50">
                    <div className="bg-indigo-50/50 border-b border-indigo-100 p-4 flex items-center gap-3">
                      <FileText className="h-5 w-5 text-indigo-600" />
                      <h3 className="font-bold text-indigo-900">{section.passage.title || 'Bacaan'}</h3>
                    </div>

                    <div className="p-6 text-slate-700 leading-relaxed font-serif text-lg whitespace-pre-wrap max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-100">
...
function QuestionCard({
  ...
  return (
    <div id={`q-${q.id}`} className={`
            group relative scroll-mt-24 rounded-2xl border bg-white p-5 sm:p-7 shadow-sm transition-all duration-300
            ${isAnswered ? 'border-indigo-200 shadow-indigo-100/50' : 'border-slate-200 hover:border-indigo-300 hover:shadow-md'}
        `}>
      ...
      <div className="flex gap-4 mb-6">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="flex items-center gap-2">
            {q.required && <span className="text-[10px] font-bold text-rose-500 bg-rose-50 px-2 py-0.5 rounded-full border border-rose-100">WAJIB</span>}
            <span className="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full border border-slate-100">{q.points} Poin</span>
          </div>
          ...
          {q.audioUrl && (
            <div className="mt-2 p-3 bg-slate-100 rounded-xl flex items-center gap-3">
              <div className="h-8 w-8 rounded-full bg-slate-800 text-white flex items-center justify-center shrink-0"><PlayCircle className="h-4 w-4" /></div>
            ...
            {q.options.map(o => {
              ...
              return (
                <label key={o.id} className={`
                            ...
                            ${checked ? 'border-indigo-500 bg-indigo-50/50 ring-1 ring-indigo-500 shadow-sm' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50'}
                  ...
                  <div className="flex-1">
                    ...
                    <span className={`${checked ? 'text-slate-900 font-medium' : 'text-slate-600'}`}>{o.text}</span>
                  </div>
                </label>
              )
            })}
          ...
          <div className="space-y-3">
            {q.options.map(o => {
              ...
              return (
                <label key={o.id} className={`
                            relative flex cursor-pointer items-start gap-4 p-4 rounded-xl border transition-all
                            ${selected ? 'border-indigo-500 bg-indigo-50/50 ring-1 ring-indigo-500 shadow-sm' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50'}
                            ${expired ? 'opacity-60 cursor-not-allowed' : ''}
                        `}>
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center mt-0.5">
                  ...
                  <div className="flex-1">
                    ...
                    <span className={`${selected ? 'text-slate-900 font-medium' : 'text-slate-600'}`}>{o.text}</span>
                  </div>
                </label>
              )
            })}
        ...
        {(q.type === 'SHORT_TEXT' || q.type === 'ESSAY') && (
          <div className="relative">
            <textarea
              ...
              className={`
                                block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 disabled:bg-slate-100 disabled:text-slate-500 sm:text-sm
                                ${answers[q.id]?.valueText ? 'bg-indigo-50/10 border-indigo-300' : ''}
                            `}
            ...
            {q.options.map(o => {
              ...
              return (
                <label key={o.id} className={`
                                    ...
                                    ${checked ? 'border-indigo-600 bg-indigo-50 text-indigo-800' : 'border-slate-200 hover:border-slate-300 text-slate-600'}
                                    ${expired ? 'opacity-60 cursor-not-allowed' : ''}
                                `}>
        ...
        {(q.type === 'NUMBER' || q.type === 'RANGE') && (
          <div className="max-w-xs">
            <input
              ...
              disabled={expired}
              placeholder="0"
              className="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-lg font-mono"
