// src/app/api/exams/[attemptId]/finish/route.ts
    ...
    function grade(q: typeof questions[number], a: (typeof answers)[number] | undefined): number {
        ...
        switch (q.type) {
            ...
            case 'MULTI_SELECT': {
                ...
                // --- SCORING PENALTY UPDATE MATCHING LIB ---
                const rightSelected = [...chosen].filter(id => correct.has(id)).length
                const wrongSelected = [...chosen].filter(id => !correct.has(id)).length
                const totalRight = correct.size

                const rawFraction = (rightSelected - wrongSelected) / totalRight
                return Math.max(0, rawFraction * q.points)
            }
            case 'RANGE': {
