...
export default async function ReviewAttemptPage({
  ...
  const numCls =
    'w-28 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30'
      ...
      <section className="mx-auto max-w-6xl space-y-6 px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-right">
            <div className="text-sm text-gray-600">Total Poin: <b>{totalPoints}</b></div>
        ...
        <form action={saveGrades} className="space-y-4">
          {questions.map(q => {
            ...
            if (!a) {
              answerBlock = <div className="text-sm text-gray-500 italic">Belum ada jawaban.</div>
              ...
              answerBlock = (
                <div className="text-sm text-gray-800">
                  ...
                  <span className="ml-3 text-gray-600">• Benar: <b>{correct || '—'}</b></span>
                </div>
              )
              ...
              // SHORT_TEXT / ESSAY
              answerBlock = (
                <div className="rounded-md border border-gray-200 bg-white p-3 text-sm text-gray-800 whitespace-pre-wrap">
                  {a?.valueText ?? '—'}
                </div>
              )
            }
              ...
              <div key={q.id} className="rounded-2xl border border-gray-200 bg-white p-4">
                ...
                <div className="mt-3 flex items-center gap-3">
                  {a ? (
                    ...
                    <span className="text-xs text-gray-500">Tidak ada jawaban — tidak bisa dinilai.</span>
                  )}
                </div>
              </div>
            )
