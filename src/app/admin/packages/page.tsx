...
const CreateSchema = z.object({
  ...
  // Boleh kosong ("") => undefined; kalau isi, harus int >= 1
  timeLimitMin: z.union([
    z.literal('').transform(() => undefined),             // kasus input kosong dari form
...
export default async function AdminPackagesPage() {
  ...
  if (!session || (session.user as any)?.role !== 'ADMIN') redirect('/login')
    ...
    async function createPackage(formData: FormData) {
      'use server'
      // pastikan nama field tepat: title, description, token, timeLimitMin
      const raw = Object.fromEntries(formData.entries())
if (raw.timeLimitMin === '') delete (raw as any).timeLimitMin
  ...
  const pkgs = await prisma.examPackage.findMany({
    ...
    select: {
      id: true, title: true, isActive: true, timeLimitMin: true,
        ...
        >
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input id="title" name="title" placeholder="Contoh: Matematika Kelas 6"
                     className="w-full rounded-xl border-black-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 p-2.5 text-black" required />
            </div>
  ...
  placeholder="Batas menit (opsional)"
                     className="w-full rounded-xl border-black-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 p-2.5 text-black" />
            ...
            <div className="sm:col-span-2 space-y-1.5">
              ...
              <textarea id="description" name="description" placeholder="Keterangan singkat paket..."
                        className="w-full rounded-xl border-black-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 p-2.5 text-black" rows={3} />
            </div>

            <div className="sm:col-span-2 space-y-1.5">
              <label htmlFor="token" className="block text-sm font-medium text-gray-800">Token paket (plaintext)</label>
              <input id="token" name="token" placeholder="Contoh: ABCD1234" autoComplete="off"
                     className="w-full rounded-xl border-black-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 p-2.5 text-black" required />
          ...
          <div className="pt-2">
            <button className="rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-5 shadow-md focus:outline-none focus:ring-4 focus:ring-blue-200 text-black">
              Simpan
            </button>
          </div>
        </form>
        ...
        <div className="space-y-3">
          ...
          <ul className="bg-white rounded-2xl shadow-lg border border-blue-100 divide-y">
            {pkgs.map((p) => (
              <li key={p.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <a href={`/admin/packages/${p.id}/results`}
                     className="inline-flex items-center justify-center rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium px-3 py-2 border border-gray-200">
                    Peserta & Nilai
                  </a>
                  ...
                  <form action={toggleActive}>
                    ...
                    <button className={`rounded-xl font-medium px-3 py-2 border
                      ${p.isActive
